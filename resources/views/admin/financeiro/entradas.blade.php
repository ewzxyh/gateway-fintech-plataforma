<x-app-layout :route="'[ADMIN] Entradas'">
          ...
          <div class="card border-danger">
            <div class="card-body p-5">
              <div class="d-flex align-items-center mb-3">
                ...
                <div>
                  <h1 class="display-6 fw-bold mb-1">Relatórios de Entradas</h1>
                  <p class="text-muted mb-0">Gerencie e monitore todas as transações de entrada do sistema</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="col-12">
          <div class="card">
            ...
            <div class="card-body p-4">
              <!-- Filtros -->
              <div class="mb-4">
                <form method="GET" action="{{ route('admin.financeiro.entradas') }}" id="filtersForm">
                  <div class="row g-3 align-items-end">
                    ...
                    <div class="col-lg-4 col-md-12">
                      <div class="d-flex gap-2 justify-content-md-start justify-content-lg-end">
                        ...
                        <a href="{{ route('admin.financeiro.entradas') }}" class="btn btn-outline-secondary">
                          <i class="fas fa-times me-1"></i>Limpar
                        </a>
                        <a href="{{ route('admin.financeiro.entradas.export', request()->query()) }}" class="btn btn-success">
              ...
              <div class="table-responsive">
                <table id="table-financeiro-entradas" class="table text-nowrap table-hover">
                  <thead>
                    <tr>
                      <th class="text-center"><i class="fa-solid fa-wallet text-primary me-1"></i>Meio</th>
                      <th class="text-center"><i class="fa-solid fa-user text-info me-1"></i>User ID</th>
                  ...
                  <tbody>
                    ...
                    <tr class="transaction-row" style="cursor: pointer;" data-transaction-id="{{ $cashOut->id }}" title="Clique para ver detalhes">
                      ...
                      <!-- Transação ID -->
                      <td class="text-center font-monospace small">{{ $cashOut->idTransaction }}</td>

                      <!-- Valor -->
                      <td class="text-center fw-semibold text-success">R$ {{ number_format($cashOut->amount, 2, ',', '.') }}</td>

                      <!-- Valor Líquido -->
                      <td class="text-center fw-semibold text-success">R$ {{ number_format($cashOut->deposito_liquido, 2, ',', '.') }}</td>
                      ...
                      <td class="text-center">
                        ...
                        <span class="badge bg-info"
                          ...
                          data-bs-content="Será liberado em {{ \Carbon\Carbon::parse($cashOut->date)->addDays($cashOut->days_availability ?? 21)->format('d/m/Y H:i') }}"
                          data-bs-placement="top">
                          <i class="fa-solid fa-hourglass-half me-1"></i>A Liberar
                        </span>
                        @break
                        @case('CANCELLED')
                        <span class="badge bg-danger">
                          <i class="fa-solid fa-times-circle me-1"></i>Cancelado
                        </span>
                        @break
                        @case('MEDIATION')
                        <span class="badge bg-warning text-dark">
                          <i class="fa-solid fa-gavel me-1"></i>Mediação
                        </span>
                        @break
                        @default
                      ...
                      <!-- Data -->
                      <td class="text-center small">{{ \Carbon\Carbon::parse($cashOut->date)->format('d/m/Y H:i') }}</td>

                      <!-- Taxa -->
                      <td class="text-center fw-semibold text-danger small">R$ {{ number_format((float)$cashOut->amount - (float)$cashOut->deposito_liquido, 2, ',', '.') }}</td>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // ==================== DATATABLE ====================
      const table = document.getElementById('table-financeiro-entradas');
      if (table) {
        ...
        if (hasData) {
          $('#table-financeiro-entradas').DataTable({
            ...
            initComplete: function() {
              $('#table-financeiro-entradas_filter input[type="search"]')
                .attr('placeholder', 'Pesquisar entradas...');
            }
          });
        } else {
    ...
    function preencherModal(dados) {
      ...
      if (dados.method === 'card') {
        ...
        <span id="modal-cartao-numero" style="">${cardNumber}</span>
      `);
        ...
        <span style="">PIX Instantâneo</span>
      `);
        ...
        <span style="">Boleto Bancário</span>
      `);
      ...
      $('#reverterMediacaoBtn').data('transaction-id', dados.id);

      // Mostrar botões apenas se o status permitir
      if (dados.status === 'PAID_OUT') {
        $('#estornarBtn').show();
        $('#mediacaoBtn').show();
        $('#reverterMediacaoBtn').hide();
      } else if (dados.status === 'MEDIATION') {
        $('#estornarBtn').hide();
        $('#mediacaoBtn').hide();
        $('#reverterMediacaoBtn').show();
      } else {
        $('#estornarBtn').hide();
        $('#mediacaoBtn').hide();
        $('#reverterMediacaoBtn').hide();
    ...
    // Função para executar reversão de mediação
    function executarReversaoMediacao(transactionId) {
      $.ajax({
        ...
        beforeSend: function() {
          console.log('Enviando requisição de reversão...');
        },
        success: function(response) {
          console.log('Resposta recebida:', response);
          alert('Mediação revertida com sucesso! O valor foi liberado e voltou ao saldo do usuário.');
          $('#detalhesModal').modal('hide');
          location.reload();
        },
        error: function(xhr, status, error) {
          ...
          console.error('Error:', error);
          console.error('Response Text:', xhr.responseText);
          alert('Erro ao reverter mediação: ' + error);
        }
      });
    }

    // Evento para botão de reversão de mediação
    $(document).on('click', '#reverterMediacaoBtn', function(e) {
      e.preventDefault();
      e.stopPropagation();

      var transactionId = $(this).data('transaction-id');
      console.log('Botão reversão mediação clicado, ID:', transactionId);
      ...
      if (confirm('Tem certeza que deseja reverter a mediação desta transação? O valor será liberado e voltará ao saldo do usuário.')) {
