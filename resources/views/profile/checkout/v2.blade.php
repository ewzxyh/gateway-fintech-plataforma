...
<body cz-shortcut-listen="true">
 ...
 <!-- Incluir checkout-v2.js -->
 <script src="{{ asset('assets-checkout/js/checkout-v2.js') }}"></script>
 
 <!-- Configurações do checkout -->
 <script>
  // Configurações globais para o checkout v2
  @php
    // Adquirente PIX (padrão para backwards compatibility)
    $default_adquirente = \App\Helpers\Helper::adquirenteDefault($produto->user_id, 'pix');
    // Adquirente para Cartão e Boleto
    $default_adquirente_card = \App\Helpers\Helper::adquirenteDefault($produto->user_id, 'card_billet');
    // Chave pública da PrimePay7
    $primepay7 = \App\Models\PrimePay7::first();
  @endphp
  
  window.checkoutV2Config = {
   checkoutId: {{ $produto->id }},
   produtoValor: {{ $produto->valor }},
   produtoName:"{{ $produto->name }}",
   redirectThankYou:"{{ $produto->obrigado_page ?? '/obrigado' }}",
   defaultAdquirente:"{{ $default_adquirente }}",
   defaultAdquirenteCard:"{{ $default_adquirente_card }}",
   primePay7PublicKey:"{{ $primepay7->public_key ?? '' }}"
  };
  
  // Configurações para compatibilidade com checkout.js v1
  window.defaultAdquirente = window.checkoutV2Config.defaultAdquirente;
  window.defaultAdquirenteCard = window.checkoutV2Config.defaultAdquirenteCard;
  window.primePay7PublicKey = window.checkoutV2Config.primePay7PublicKey;
  window.redirect_tankyou = window.checkoutV2Config.redirectThankYou;
  window.produto_valor = window.checkoutV2Config.produtoValor;
  
  console.log('Configurações do checkout v2:', window.checkoutV2Config);
 </script>
