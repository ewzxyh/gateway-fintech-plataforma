...
use App\Http\Controllers\Api\Adquirentes\TrustyPixController;
use App\Http\Controllers\Api\Adquirentes\RedeController;
use App\Http\Controllers\RedeTokenizationController;
...
Route::middleware(['api.logger', 'check.token.secret'])->group(function () {
...
Route::middleware(['check.token.secret', 'check.allowed.ip', 'throttle:30,1'])->post('wallet/saque/payment', [SaqueController::class, 'makePayment']);
Route::middleware(['check.token.secret', 'check.allowed.ip', 'check.pin', 'throttle:30,1'])->post('pixout', [SaqueController::class, 'makePayment']);
...
/* TRUSTYPIX CALLBACKS */
Route::post('trustypix/callback/deposit', [TrustyPixController::class, 'callbackDeposit'])->middleware(['validate.webhook', 'throttle:30,1']);
Route::post('trustypix/callback/withdraw', [TrustyPixController::class, 'callbackWithdraw'])->middleware(['validate.webhook', 'throttle:30,1']);
Route::post('trustypix/test', [TrustyPixController::class, 'testCallback'])->middleware(['validate.webhook', 'throttle:10,1']);

/* REDE CALLBACKS */
Route::post('rede/callback/deposit', [RedeController::class, 'callbackDeposit'])->middleware(['validate.webhook', 'throttle:30,1']);
Route::post('rede/callback/withdraw', [RedeController::class, 'callbackWithdraw'])->middleware(['validate.webhook', 'throttle:30,1']);
Route::post('rede/callback', [RedeController::class, 'callbackUnified'])->middleware(['validate.webhook', 'throttle:30,1']);
Route::post('rede/webhook', [RedeController::class, 'webhook'])->middleware(['validate.webhook', 'throttle:30,1']);

/* STRIPE WEBHOOKS */
Route::post('stripe/webhook', [App\Http\Controllers\Api\Adquirentes\StripeController::class, 'webhook'])->middleware(['throttle:30,1']);

/* STRIPE STATUS CHECK */
Route::get('stripe/status', [App\Http\Controllers\Api\StripeStatusController::class, 'checkStatus'])->middleware(['throttle:60,1']);

/* REDE TOKENIZATION */
Route::middleware(['check.token.secret', 'throttle:60,1'])->group(function () {
    Route::post('rede/tokenization', [RedeTokenizationController::class, 'create']);
    Route::get('rede/tokenization/{tokenizationId}', [RedeTokenizationController::class, 'show']);
    Route::put('rede/tokenization/{tokenizationId}/status', [RedeTokenizationController::class, 'updateStatus']);
    Route::post('rede/tokenization/{tokenizationId}/cryptogram', [RedeTokenizationController::class, 'generateCryptogram']);
    Route::put('rede/tokenization/{tokenizationId}/callback', [RedeTokenizationController::class, 'setCallback']);
    Route::post('rede/tokenization/callback-url', [RedeTokenizationController::class, 'setCallbackUrl']);
});

/* LOGO ROUTES */
Route::get('logos', [LogoController::class, 'getLogos']);
Route::get('logos/{theme}', [LogoController::class, 'getLogoByTheme'])->where('theme', 'light|dark');