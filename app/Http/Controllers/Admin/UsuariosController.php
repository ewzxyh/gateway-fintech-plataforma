...
class UsuariosController extends Controller
{
    public function index(Request $request)
    {
        $users = User::where('id', '>=', 1)
                    ->where('user_id', '!=', 'gateway_wallet'); // Excluir usuário Gateway Wallet
        ...
        if (isset($buscar)) {
            $users->where(function($query) use ($buscar) {
                $query->where('name', "LIKE", "%$buscar%")
                      ->orWhere('username', "LIKE", "%$buscar%")
                      ->orWhere('email', "LIKE", "%$buscar%")
                      ->orWhere('cpf_cnpj', "LIKE", "%$buscar%")
                      ->orWhere('user_id', "LIKE", "%$buscar%");
            });
        }

        $users = $users->get(); // executa a query e pega os resultados
        $gerentes = User::where('permission', 5)->where('user_id', '!=', 'gateway_wallet')->get();
