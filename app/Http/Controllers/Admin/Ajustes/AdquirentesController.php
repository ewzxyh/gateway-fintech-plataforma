...
class AdquirentesController extends Controller
{
    ...
    public function updateRede(Request $request)
    {
        $request->validate([
            'pv' => 'required|string',
            'token' => 'required|string',
        ...
        // Campos que devem ser salvos como string
        $stringFields = ['pv', 'token', 'environment', 'api_url', 'webhook_url'];
        ...
        $rede = Rede::first();
        if (!$rede) {
            $rede = Rede::create($payload);
        } else {
            $rede->update($payload);
        }

        return back()->with('success', 'Dados da Rede e.Rede alterados com sucesso!');
    }

    /**
     * Atualiza configurações do Stripe
     */
    public function updateStripe(Request $request)
    {
        ...
        // Campos que devem ser salvos como string
        $stringFields = ['publishable_key', 'secret_key', 'webhook_secret', 'environment', 'webhook_url', 'currency', 'country'];
        ...
        $stripe = \App\Models\Stripe::first();
        if (!$stripe) {
            $stripe = \App\Models\Stripe::create($payload);
        } else {
            $stripe->update($payload);
        }

        return back()->with('success', 'Dados do Stripe alterados com sucesso!');
    }

    public function updateTrustyPix(Request $request)
    {
        $request->validate([
            'url' => 'required|string',
            'client_id' => 'required|string',
        ...
        // Campos que devem ser salvos como string
        $stringFields = ['url', 'client_id', 'client_secret'];
        ...
        $trustypix = TrustyPix::first();
        if (!$trustypix) {
            $trustypix = TrustyPix::create($payload);
        } else {
            $trustypix->update($payload);
        }

        return back()->with('success', 'Dados da TrustyPix alterados com sucesso!');
    }
}
