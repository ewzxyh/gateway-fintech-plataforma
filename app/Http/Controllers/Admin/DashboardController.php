...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $periodo = $request->input('periodo', null); // Mudança: null como padrão
        
        // Se não há período na URL, usar filtros globais do localStorage (via JavaScript)
        // O JavaScript irá recarregar a página com o período correto
        if ($periodo === null) {
            // Usar período padrão se não há filtros globais
            $periodo = 'tudo';
        }
        ...
        // === LUCRO LÍQUIDO ===
        // Calcular lucro líquido total independente do período
        // Similar ao Helper::calculaSaldoLiquido() para usuários
        $lucroLiquidoData = Helper::calcularLucroLiquidoTotal();
        $lucro_liquido = $lucroLiquidoData['lucro_liquido_total'];
        $lucroDepositos = $lucroLiquidoData['lucro_depositos'];
        $lucroSaques = $lucroLiquidoData['lucro_saques'];
        $taxasAdquirentesEntradas = $lucroLiquidoData['taxas_adquirentes_entradas'];
        $taxasAdquirentesSaidas = $lucroLiquidoData['taxas_adquirentes_saidas'];

        // Queries para outras métricas (ainda baseadas no período)
        ...
        // === CARTEIRA GATEWAY ===
        // Calcular saldo total da carteira gateway independente do período
        // Similar ao Helper::calculaSaldoLiquido() para usuários
        $gatewayWalletData = GatewayWallet::calcularSaldoTotal();
        
        // Criar objeto gateway wallet com os dados calculados para compatibilidade
        $gatewayWallet = new GatewayWallet();
        ...
        $gatewayWalletData = [
            'taxa_sistema_total' => $gatewayWalletData['taxa_sistema_total'],
            'taxa_adquirente_total' => $gatewayWalletData['taxa_adquirente_total'],
            'saldo_gateway' => $gatewayWalletData['saldo_gateway_total'],
            'saldo_disponivel' => $gatewayWalletData['saldo_disponivel'],
            'saldo_pendente' => $gatewayWalletData['saldo_pendente'],
            'total_sacado' => $gatewayWalletData['total_sacado'],
            'transacoes_count' => $transacoes_aprovadas
        ];
        ...
        return view("admin.dashboard", compact(
            ...
            "saques_pendentes",
            "gatewayWalletData",
            "taxasAdquirentesEntradas",
            "taxasAdquirentesSaidas"
