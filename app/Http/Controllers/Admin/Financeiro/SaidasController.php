...
class SaidasController extends Controller
{
    ...
    {
        try {
            Log::info('getSaidaDetails (Admin) chamado com ID: ' . $id);
            ...
            if (!$saida) {
                Log::warning('Saída não encontrada. ID: ' . $id);
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'valor_liquido' => number_format($saida->cash_out_liquido, 2, ',', '.'),
                    'taxa' => number_format((float)$saida->amount - (float)$saida->cash_out_liquido, 2, ',', '.'),
                    'beneficiario' => $saida->beneficiaryname ?? 'N/A',
                    'documento' => $saida->beneficiarydocument ?? 'N/A',
                    'status' => $saida->status,
                    'data' => Carbon::parse($saida->date)->format('d/m/Y H:i'),
                    'idTransaction' => $saida->externalreference ?? $saida->idTransaction ?? 'N/A',
                    'chavePix' => $saida->pixkey ?? 'N/A',
                    'tipoChave' => $saida->pixkey ?? 'N/A'
                ]
            ]);

        } catch (\Exception $e) {
            Log::error('Erro ao buscar detalhes da saída (Admin): ' . $e->getMessage());
            return response()->json([
                'success' => false,
