...
class DashboardControlller extends Controller
{
    public function index(Request $request)
    {
        ...
        $periodoFiltro = $request->input('periodo', null); // Mudança: null como padrão
        ...
        if ($periodoFiltro === null) {
            // Usar período padrão do mês atual se não há filtros globais
            $periodoFiltro = 'tudo';
        }
        ...
        } elseif ($vendasAnterior > 0 && $vendasAtual == 0) {
            $crescimentoVendas = -100; // Se havia vendas antes e agora não há
        }
        
        // Limitar a porcentagem para evitar valores extremos
        $crescimentoVendas = max(-100, min(100, $crescimentoVendas));
