...
class RelatoriosControlller extends Controller
{
    ...
    {
        try {
            Log::info('getTransactionDetails chamado com ID: ' . $id);
            
            // Validar ID
            if (!is_numeric($id) || $id <= 0) {
                Log::warning('ID inválido fornecido: ' . $id);
            ...
            $userId = $user->user_id;
            Log::info('Buscando transação ID: ' . $id . ' para usuário: ' . $userId);
            ...
            if (!$solicitacao) {
                Log::warning('Transação não encontrada. ID: ' . $id . ', User ID: ' . $userId);
                return response()->json(['success' => false, 'message' => 'Transação não encontrada'], 404);
            }
            
            Log::info('Transação encontrada: ' . $solicitacao->idTransaction);
    ...
    public function getSaidaDetails($id)
    {
        try {
            Log::info('getSaidaDetails chamado com ID: ' . $id);
            ...
            $userId = $user->user_id;
            Log::info('Buscando saída ID: ' . $id . ' para usuário: ' . $userId);
            
            $saida = DB::table('solicitacoes_cash_out')
                ->where('id', (int)$id)
                ->where('user_id', $userId)
                ->first();

            if (!$saida) {
                Log::warning('Saída não encontrada. ID: ' . $id . ', User ID: ' . $userId);
                return response()->json(['success' => false, 'message' => 'Transação não encontrada'], 404);
            }
            
            Log::info('Saída encontrada: ' . ($saida->externalreference ?? $saida->idTransaction));
            
            return response()->json([
                'success' => true,
                'data' => [
                    'id' => $saida->id,
                    'valor' => number_format($saida->amount, 2, ',', '.'),
                    'taxa' => number_format($saida->taxa_cash_out ?? 0, 2, ',', '.'),
                    'beneficiario' => $saida->beneficiaryname ?? 'N/A',
                    'status' => $saida->status,
                    'data' => Carbon::parse($saida->date)->format('d/m/Y H:i'),
                    'idTransaction' => $saida->externalreference ?? $saida->idTransaction ?? 'N/A',
                    'chavePix' => $saida->pix ?? 'N/A',
                    'tipoChave' => $saida->pixkey ?? 'N/A'
                ]
            ]);

        } catch (\Exception $e) {
            Log::error('Erro ao buscar detalhes da saída: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Erro ao buscar detalhes da transação: ' . $e->getMessage()
            ], 500);
