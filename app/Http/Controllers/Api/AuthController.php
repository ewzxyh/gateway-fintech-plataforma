...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            // Buscar usuário pelo username (com ou sem @) ou email
            $user = User::where('username', $username)
                       ->orWhere('username', '@' . $username)
                       ->orWhere('username', ltrim($username, '@'))
            ...
            // Buscar as chaves do usuário (normalizar username para buscar chaves)
            $keyUsername = ltrim($user->username, '@');
            $userKeys = UsersKey::where('user_id', $keyUsername)->first();
    ...
    public function verify2FA(Request $request)
    {
        try {
            ...
            if (!$userKeys) {
                Log::info('Usuário sem chaves de API - criando automaticamente (2FA)', [
                    'username' => $user->username,
                    'ip' => $request->ip()
                ]);
                ...
                Log::info('Chaves de API criadas automaticamente (2FA)', [
                    'username' => $user->username,
                    'token' => $token,
                    'ip' => $request->ip()
                ]);
