...
class CallbackController extends Controller
{
    ...
    public function callbackWoovi(Request $request)
    {
        try {
            ...
            $event = $data['event'] ?? '';
            $allowedEvents = ['charge.completed', 'OPENPIX:CHARGE_COMPLETED', 'OPENPIX:CHARGE_EXPIRED'];
            
            if (!in_array($event, $allowedEvents)) {
                Log::info('[WOOVI][CALLBACK]: Evento não processável, ignorando. Evento recebido: ' . $event, []);
            ...
            // Tratar evento de expiração de cobrança
            if ($event === 'OPENPIX:CHARGE_EXPIRED') {
                ...
                Log::info('[WOOVI][CALLBACK]: Cobrança expirada', [
                    'correlationID' => $correlationID,
                    'event' => $event
                ]);

                return response()->json(['status' => 'success', 'message' => 'Cobrança marcada como expirada']);
            }
        ...
        if ($transacao->callback && $transacao->callback != 'web') {
            $payload = [
                ...
                "debtor_document_number" => $transacao->client_document ?? '08355037120',
    ...
    public function callbackWooviWithdraw(Request $request)
    {
        try {
            ...
            $event = $data['event'] ?? '';
            $allowedWithdrawEvents = [
                'withdraw.completed', 
                'OPENPIX:WITHDRAW_COMPLETED', 
                'OPENPIX:MOVEMENT_CONFIRMED',
                'OPENPIX:MOVEMENT_FAILED'
            ];
            
            if (!in_array($event, $allowedWithdrawEvents)) {
                Log::info('[WOOVI][WITHDRAW_CALLBACK]: Evento não processável, ignorando. Evento recebido: ' . $event, []);
                return response()->json(['status' => 'success', 'message' => 'Evento ignorado']);
            }
            ...
            // Tratar evento de falha de saque
            if ($event === 'OPENPIX:MOVEMENT_FAILED') {
                ...
                // Buscar o usuário para devolver o saldo
                $user = User::where('user_id', $cashout->user_id)->first();
                if ($user) {
                    // Devolver o saldo ao usuário (o saldo foi decrementado na solicitação)
                    Helper::incrementAmount($user, $cashout->amount, 'saldo');
                    Helper::calculaSaldoLiquido($user->user_id);

                    Log::info('[WOOVI][WITHDRAW_CALLBACK]: Saldo devolvido ao usuário após falha no saque', [
                        'user_id' => $user->user_id,
                        'amount' => $cashout->amount
                    ]);

                    // Log de devolução de saldo
                    \App\Helpers\BalanceLogHelper::logWithdrawalOperation(
                        ...
                        [
                            ...
                            'motivo' => 'Falha no processamento do saque',
                            'operacao' => 'woovi_withdraw_failed'
                        ]
                    );
                }
                ...
                if ($cashout->callback && $cashout->callback != 'web') {
                    $payload = [
                        ...
                        "failedAt" => Carbon::now()->toISOString(),
                        "message" => "Falha no processamento do saque"
                    ];

                    Log::info('[WOOVI][WITHDRAW_CALLBACK]: Enviando webhook de falha para cassino', [
                    ...
                    try {
                        ...
                        Log::info('[WOOVI][WITHDRAW_CALLBACK]: Webhook de falha enviado para cassino', [
                            'callback_url' => $cashout->callback,
                            'status_code' => $response->status(),
                            'response' => $response->body()
                        ]);
                    } catch (\Exception $e) {
                        Log::error('[WOOVI][WITHDRAW_CALLBACK]: Erro ao enviar webhook de falha para cassino', [
                            'callback_url' => $cashout->callback,
                            'error' => $e->getMessage()
                        ]);
                    }
                ...
                Log::info('[WOOVI][WITHDRAW_CALLBACK]: Saque marcado como falho', [
                    'transactionId' => $transactionId,
                    'event' => $event
                ]);

                return response()->json(['status' => 'success', 'message' => 'Saque marcado como falho e saldo devolvido']);
            }
