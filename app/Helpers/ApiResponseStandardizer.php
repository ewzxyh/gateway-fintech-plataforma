...
class ApiResponseStandardizer
{
     ...
     */
    public static function standardizeWithdrawResponse($response, $amount = null, $request = null)
    {
        $standardized = [
            'status' => 'success',
        ...
        // Se não tem pixKey e pixKeyType na resposta, tentar obter do request
        if (!$standardized['pixKey'] && $request && method_exists($request, 'input')) {
            $standardized['pixKey'] = $request->input('pixKey');
        }
        
        if (!$standardized['pixKeyType'] && $request && method_exists($request, 'input')) {
            $standardized['pixKeyType'] = $request->input('pixKeyType');
        }
    ...
    public static function standardizeErrorResponse($response, $defaultMessage = 'Erro interno do servidor')
    {
        ...
        if (isset($response['message'])) {
            ...
            if (is_array($response['errors'])) {
                $standardized['message'] = implode(', ', array_map(function($error) {
