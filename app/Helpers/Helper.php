...
class Helper
{
    ...
    /**
     * Retorna a logo correta baseada no tema atual
     * 
     * @param string|null $theme Tema atual ('dark' ou 'light')
     * @return string Caminho da logo
     */
    public static function getLogoByTheme($theme = null)
    {
        $setting = App::first();
        
        if (!$setting) {
            return '/assets/images/default-logo.png'; // Logo padrão caso não encontre configurações
        }
        
        // Se não foi passado o tema, tenta detectar do localStorage via JavaScript
        if (!$theme) {
            // Por padrão, usa a logo clara se não conseguir detectar
            return asset($setting->gateway_logo);
        }
        
        // Retorna a logo baseada no tema
        if ($theme === 'dark' && $setting->gateway_logo_dark) {
            return asset($setting->gateway_logo_dark);
        }
        
        // Fallback para logo clara
        return asset($setting->gateway_logo);
    }
    ...
    public static function incrementAmount(User $user, $valor, $campo)
{
    ...
    /**
     * Calcula o lucro líquido total da plataforma
     * Similar ao calculaSaldoLiquido() mas para toda a plataforma
     * 
     * @return array Array com os dados do lucro líquido
     */
    public static function calcularLucroLiquidoTotal()
    {
        try {
            Log::info('=== HELPER::calcularLucroLiquidoTotal INICIADO ===');
            
            // === LUCRO DE DEPÓSITOS ===
            // Soma total dos depósitos líquidos (valor que entra na plataforma)
            $totalDepositoLiquido = Solicitacoes::where('status', 'PAID_OUT')
                ->sum('deposito_liquido');
            
            // Soma total dos valores brutos dos depósitos
            $totalDepositoBruto = Solicitacoes::where('status', 'PAID_OUT')
                ->sum('amount');
            
            // Lucro dos depósitos = diferença entre valor bruto e líquido
            // CORREÇÃO: Evitar valores negativos quando deposito_liquido > amount
            $lucroDepositos = (float) $totalDepositoBruto - (float) $totalDepositoLiquido;
            if ($lucroDepositos < 0) {
                $lucroDepositos = 0; // Evitar valores negativos
            }
            
            Log::info('calcularLucroLiquidoTotal: Depósitos', [
                'total_deposito_bruto' => $totalDepositoBruto,
                'total_deposito_liquido' => $totalDepositoLiquido,
                'lucro_depositos' => $lucroDepositos
            ]);
            
            // === LUCRO DE SAQUES ===
            // Soma total dos saques pagos (valor que sai da plataforma)
            $totalSaquesPagos = SolicitacoesCashOut::whereIn('status', ['PAID_OUT', 'COMPLETED'])
                ->sum(\DB::raw('amount + taxa_cash_out'));
            
            // Lucro dos saques = taxas cobradas nos saques
            $lucroSaques = SolicitacoesCashOut::whereIn('status', ['PAID_OUT', 'COMPLETED'])
                ->sum('taxa_cash_out');
            
            Log::info('calcularLucroLiquidoTotal: Saques', [
                'total_saques_pagos' => $totalSaquesPagos,
                'lucro_saques' => $lucroSaques
            ]);
            
            // === TAXAS DE ADQUIRENTES ===
            // Calcular taxas pagas para adquirentes nas entradas
            ...
            if ($xdpag && $xdpag->taxa_adquirente_entradas > 0) {
                ...
                $taxasAdquirentesEntradas += $query->sum(\DB::raw('amount * ' . ($xdpag->taxa_adquirente_entradas / 100)));
            }
            ...
            if ($woovi && $woovi->taxa_adquirente_entradas > 0) {
                ...
                $taxasAdquirentesEntradas += $query->sum(\DB::raw('amount * ' . ($woovi->taxa_adquirente_entradas / 100)));
            }
            ...
            if ($witetec && $witetec->taxa_adquirente_entradas > 0) {
                ...
                $taxasAdquirentesEntradas += $query->sum(\DB::raw('amount * ' . ($witetec->taxa_adquirente_entradas / 100)));
            }
            ...
            if ($bspay && $bspay->taxa_adquirente_entradas > 0) {
                ...
                $taxasAdquirentesEntradas += $query->sum(\DB::raw('amount * ' . ($bspay->taxa_adquirente_entradas / 100)));
            }
            ...
            if ($pixup && $pixup->taxa_adquirente_entradas > 0) {
                ...
                $taxasAdquirentesEntradas += $query->sum(\DB::raw('amount * ' . ($pixup->taxa_adquirente_entradas / 100)));
            }
            ...
            if ($trustypix && $trustypix->taxa_adquirente_entradas > 0) {
                ...
                $taxasAdquirentesEntradas += $query->sum(\DB::raw('amount * ' . ($trustypix->taxa_adquirente_entradas / 100)));
            }
            
            // Calcular taxas de saídas (se houver)
            if ($xdpag && $xdpag->taxa_adquirente_saidas > 0) {
                $query = SolicitacoesCashOut::whereIn('status', ['PAID_OUT', 'COMPLETED'])
                ...
                $taxasAdquirentesSaidas += $query->sum(\DB::raw('amount * ' . ($xdpag->taxa_adquirente_saidas / 100)));
            }
            
            Log::info('calcularLucroLiquidoTotal: Taxas Adquirentes', [
                'taxas_adquirentes_entradas' => $taxasAdquirentesEntradas,
                'taxas_adquirentes_saidas' => $taxasAdquirentesSaidas
            ]);
            
            // === LUCRO LÍQUIDO TOTAL ===
            // Lucro total = Lucro dos depósitos + Lucro dos saques - Taxas de adquirentes
            $lucroLiquidoTotal = (float) $lucroDepositos + (float) $lucroSaques - (float) $taxasAdquirentesEntradas - (float) $taxasAdquirentesSaidas;
            
            Log::info('calcularLucroLiquidoTotal: Cálculo final', [
                'lucro_depositos' => $lucroDepositos,
                'lucro_saques' => $lucroSaques,
                'taxas_adquirentes_entradas' => $taxasAdquirentesEntradas,
                'taxas_adquirentes_saidas' => $taxasAdquirentesSaidas,
            ...
            Log::info('=== HELPER::calcularLucroLiquidoTotal FINALIZADO ===');
            ...
            Log::error('Erro em calcularLucroLiquidoTotal', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            ...
            return [
                ...
                'lucro_depositos' => 0,
                'lucro_saques' => 0,
                'taxas_adquirentes_entradas' => 0,
                'taxas_adquirentes_saidas' => 0,
                'total_deposito_bruto' => 0,
                'total_deposito_liquido' => 0,
