...
trait PinManagementTrait
{
    ...
    /**
     * Cria um PIN para o usuário (método interno)
     */
    public function createPinInternal(User $user, ?string $pin = null): array
    {
        try {
            if (!$pin) {
     ...
     */
    public function togglePinStatus(User $user, bool $active): array
    {
        try {
            if ($active && !$user->pin) {
                return [
    ...
    /**
     * Altera o PIN do usuário (método interno)
     */
    public function changePinInternal(User $user, string $currentPin, string $newPin): array
    {
        try {
            // Verificar se o PIN atual está correto
            if (!$this->verifyPinInternal($user, $currentPin)) {
                return [
                    'success' => false,
    ...
    /**
     * Verifica se o PIN está correto (método interno)
     */
    public function verifyPinInternal(User $user, string $pin): bool
    {
     ...
     */
    public function hasActivePin(User $user): bool
    {
     ...
     */
    public function getPinInfo(User $user): array
    {
    ...
    /**
     * Obtém informações do PIN do usuário (método estático para uso em views)
     */
    public static function getPinInfoStatic(User $user): array
    {
        $createdAt = $user->pin_created_at;
        if ($createdAt && is_string($createdAt)) {
            $createdAt = Carbon::parse($createdAt);
    ...
    /**
     * Remove o PIN do usuário (método interno)
     */
    public function removePinInternal(User $user, string $currentPin): array
    {
        try {
            // Verificar se o PIN atual está correto
            if (!$this->verifyPinInternal($user, $currentPin)) {
