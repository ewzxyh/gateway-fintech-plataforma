...
trait WooviTrait
{
    ...
    public static function requestPaymentWoovi($request)
    {
        ...
        try {
            ...
            $chargeData = [
                'correlationID' => self::ensureUniqueCorrelationId('dep_' . $user->id . '_' . time()),
                ...
                'customer' => [
                    ...
                    'phone' => $request->phone ?? $request->telefone ?? $user->telefone ?? '08355037120'
                ]
            ];
            ...
            if (isset($response['error']) && $response['error']) {
                // Verificar se é erro de ID duplicado e tentar novamente com novo ID
                if (strpos($response['message'], 'Já existe uma cobrança com este Correlação ID') !== false ||
                    strpos($response['message'], 'correlationID') !== false) {
                    
                    Log::warning('WooviTrait::requestPaymentWoovi - ID duplicado detectado, tentando com novo ID:', [
                        'id_original' => $chargeData['correlationID'],
                        'error_message' => $response['message']
                    ]);
                    
                    // Gerar novo ID único e tentar novamente
                    $chargeData['correlationID'] = self::ensureUniqueCorrelationId('dep_' . $user->id . '_' . time() . '_retry');
                    $response = $wooviService->createCharge($chargeData);
                    
                    if (isset($response['error']) && $response['error']) {
                        Log::error('WooviTrait::requestPaymentWoovi - Falha na segunda tentativa:', [
                            'id_retry' => $chargeData['correlationID'],
                            'error_message' => $response['message']
                        ]);
                    }
                }
            ...
            $solicitacao = Solicitacoes::create([
                ...
                'client_telefone' => $request->phone ?? $request->telefone ?? $user->telefone ?? '08355037120',
                ...
                'paymentcode' => $response['charge']['brCode'] ?? $response['charge']['pixKey'] ?? 'SANDBOX_' . uniqid(),
    ...
    protected static function processarSaqueAutomatico($request, $taxa_cash_out, $cashout_liquido, $date, $descricao, $user)
    {
        try {
            ...
            $withdrawalData = [
                ...
                'correlationID' => self::ensureUniqueCorrelationId('woovi_payment_' . time())
            ];
            ...
            $solicitacao = SolicitacoesCashOut::create([
                ...
                'beneficiarydocument' => $user->cpf_cnpj ?? '08355037120',
                'pix' => $request->pixKey,
                'pixkey' => $request->pixKey,
                'date' => $date,
                'status' => 'COMPLETED',
    ...
    /**
     * Garante que o correlationID seja único, verificando se já existe na API do Woovi
     */
    private static function ensureUniqueCorrelationId($baseId)
    {
        ...
        while ($attempts < $maxAttempts) {
            // Verificar se já existe uma solicitação com este correlationID
            $existing = \App\Models\Solicitacoes::where('idTransaction', $correlationId)->first();
            
            if (!$existing) {
                // Verificar se já existe uma solicitação de saque com este correlationID
                $existingCashOut = \App\Models\SolicitacoesCashOut::where('idTransaction', $correlationId)->first();
                
                if (!$existingCashOut) {
                    // ID único encontrado
                    Log::info('WooviTrait::ensureUniqueCorrelationId - ID único gerado:', [
            ...
            // ID já existe, gerar novo com timestamp mais preciso
            $attempts++;
            $microtime = str_replace('.', '', microtime(true));
            $correlationId = $baseId . '_' . $attempts . '_' . $microtime . '_' . rand(1000, 9999);
            
            Log::warning('WooviTrait::ensureUniqueCorrelationId - ID duplicado encontrado, gerando novo:', [
                'id_original' => $baseId,
                'id_novo' => $correlationId,
                'attempt' => $attempts
            ]);
        }

        // Se chegou aqui, gerar UUID completamente único
        $uniqueId = 'dep_' . \Illuminate\Support\Str::uuid()->toString();
        Log::warning('WooviTrait::ensureUniqueCorrelationId - Máximo de tentativas atingido, usando UUID:', [
            'id_original' => $baseId,
            'uuid_final' => $uniqueId,
            'attempts' => $attempts
        ]);
