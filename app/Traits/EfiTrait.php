...
trait EfiTrait
{
    ...
    private static function ensureUniqueTransactionId($requestedId, $userId)
    {
        ...
        if (in_array($existing->status, ['CANCELLED', 'FAILED', 'REJECTED'])) {
            ...
            Log::warning('EfiTrait::ensureUniqueTransactionId - ID duplicado encontrado (status: ' . $existing->status . '), gerando novo:', [
        ...
        Log::warning('EfiTrait::ensureUniqueTransactionId - ID duplicado em uso ativo, gerando UUID novo:', [
            'id_original' => $requestedId,
            'id_novo' => $newId,
            'user_id' => $userId,
            'registro_existente_id' => $existing->id,
